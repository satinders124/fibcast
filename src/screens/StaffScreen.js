...
function StaffCard({ member, onRemove }) {
  return (
    <View style={s.staffCard}>
      ...
      <TouchableOpacity onPress={() => onRemove(member)} style={s.removeBtn} activeOpacity={0.7}>
  ...
  if (!isOwner) {
    return (
      <SafeAreaView style={s.safe}>
        <EmptyState icon="🔒" title="Owner Access Only" subtitle="Only the account owner can manage staff members." />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      >
        <View style={s.header}>
          ...
          <TouchableOpacity style={s.addBtn} onPress={() => setShowForm(!showForm)} activeOpacity={0.7}>
        ...
        <View style={s.section}>
          ...
          {staff.length === 0 ? (
            <EmptyState icon="👥" title="No staff yet" subtitle="Add staff members so your team can manage customers." onAction={() => setShowForm(true)} actionLabel="+ Add First Staff" />
          ) : (
            <Card>
              {staff.map((member, i) => (
